...
- `round_down_from()` now reads `threshold` the way `round_up_from()` does. It rounds *down* when the part cut off by rounding is at most `threshold` tenths of a step, where it used to round down when that part was at most `10 - threshold` tenths --- it was the point reflection of `round_up_from()` rather than its partner at the same threshold. The two agreed at the `5` that `round_up()` and `round_down()` round from, and nowhere else, so `round_down_from(4.28, 1, threshold = 9)` was `4.3` and is now `4.2`. Anything at the default threshold is unaffected, as are `round_up_from()`, `round_up()`, `round_down()`, and every `round_ties_*()` function.
  ...
  The reason for the change is what the old pairing did to `rounding = "up_from_or_down_from"`, whose range is the union of its two constituents'. Since those constituents switched direction at opposite ends of the step, the union came to as much as 1.8 rounding steps --- wider than the preimage of any one rounding procedure, and so a reconstruction that no rounding of any kind could produce. GRIM under `threshold = 9` accepted *every* mean at `n = 57`; the test decided nothing at all. The three `"*_from"` methods now span exactly one step at every threshold, as `"up"`, `"down"`, and `"up_or_down"` do at the 5 they are fixed to, and `threshold` moves a reconstructed range without widening it. Consistency verdicts under `"up_from"`, `"down_from"`, and `"up_from_or_down_from"` with a threshold other than 5 therefore change, in `grim()`, `grimmer()`, `debit()`, their mappers, and `unround()`. `symmetric` mirrors the threshold along with the method it applies to, which it did not have to do while the two directions were reflections of each other.

- `reround()` now requires `digits` and `x` to have the same length unless either of them has length 1. R's own recycling rules passed a shorter `digits` without a word and rounded the extra values at the wrong decimal level, which is the bug `unround()` was fixed for in this release; `reround()` never had the matching check. A length that is a whole multiple of `length(x)` used to recycle silently and is now an error too, in keeping with the tidyverse rule the rest of the package follows.
...
- New functions `round_ties_up()`, `round_ties_down()`, `round_ties_away()`, `round_ties_zero()`, and `round_ties_even()`, and matching `rounding` strings `"ties_up"`, `"ties_down"`, `"ties_away"`, `"ties_zero"`, and `"ties_even"`. Each names a complete tie-breaking procedure, so it says by itself what `rounding` and `symmetric` say together: `"ties_up"` is `"up"` with `symmetric = FALSE`, `"ties_away"` is `"up"` with `symmetric = TRUE`, and likewise for the other two. Since the name already determines the procedure, `symmetric` must not be given alongside one of these strings; doing so is an error, and the message names the string that was meant. `reround(-2.5, 0, "ties_up", symmetric = TRUE)` looks like a request for ties away from zero but could only have honored half of it, so it points at `"ties_away"` instead. This applies wherever the strings are accepted, including `unround()` and the mappers, and `unround()`'s vectorized `rounding` makes a single `"ties_*"` element enough to reject a `symmetric` of `TRUE` for the whole call.
  ...
  `round_ties_even()` and `"ties_even"` are the odd pair out: they are another spelling of `base::round()` and of `rounding = "even"`, which no combination of a threshold and `symmetric` produces, since parity is not a direction. They are here because *roundTiesToEven* is IEEE 754's default rounding direction and the one R, Python, and NumPy take, which made it the conspicuous absence from a family of names whose whole point is naming the tie rule. With it, the five `"ties_*"` strings together with `"ceiling"`, `"floor"`, and `"trunc"` name every rounding direction the standard defines.
