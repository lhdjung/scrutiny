...
#'
#' @description `debit_map_disperse()` extends DEBIT to cases where only group
#'   means and standard deviations (SDs) were reported, not group sizes.
#'
#'   The function is analogous to `grim_map_disperse()`, relying on the same
#'   infrastructure.
...
#' @include debit-map.R function-map-disperse.R
...
#'   generic, so you can call `audit()` following up on `debit_map_disperse()`
...
debit_map_disperse <- function_map_disperse(
  .fun = debit_map,
  .reported = c("x", "sd"),
  .name_test = "DEBIT",
  .name_class = "scr_debit_map_disperse"
)
