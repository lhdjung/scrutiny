...
#'
#' @description `function_map_disperse()` is the engine that powers functions
#'   such as `grim_map_disperse()`. It creates new, "manufactured" functions for
...
#'   | ---                            | ---                                                   | ---
#'   | `grim_map_disperse()`          | `x` (means or proportions)                            | `vignette("grim")`
#'   | `debit_map_disperse()`         | `x` (binary means); `sd` (binary standard deviations) | `vignette("debit")`

#' @section Conventions: The name of the manufactured function should
#'   mechanically follow from that of the input function. For example,
#'   `grim_map_disperse()` derives from `grim_map()`. This pattern fits best if
#'   the input function itself is named after the test it performs on a data
#'   frame, followed by `_map`: `grim_map()` applies GRIM, `debit_map()` applies
#'   DEBIT, etc.
#'
#'   Much the same is true for the classes of data frames returned by the
#'   manufactured function via the `.name_class` argument of
#'   `function_map_disperse()`. It should be the function's own name preceded by
#'   the name of the package that contains it or by an acronym of that package's
#'   name. In this way, existing classes are `scr_grim_map_disperse` and
#'   `scr_debit_map_disperse`.
#'
#'   Consider writing an `audit()` method for every such class, as this is their
#'   main purpose. The method should simply call `summarize_map_disperse()`,
