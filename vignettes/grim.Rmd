...
2.  Testing the values around reported means or proportions.
...
```{r}
grim(x = 5.27, n = 43, digits_x = 2)
```

Note `digits_x`, the number of decimal places in the reported mean. You always need to state it, and it needs to be correct: the number of decimal places is crucial for GRIM-testing, but a numeric value doesn't preserve it. A mean reported as 5.30 is stored as `5.3`, so scrutiny cannot tell it apart from a mean that really was reported as 5.3. Only you can, which is why `digits_x` has no default.
...
Use RStudio's multiple cursors to set commas at the end of all the `x` values. See `vignette("wrangling")`, section *With copy and paste*, if you are not sure how to do that.

Now, run `grim_map()` on that data frame, stating that the means were reported with two decimal places:

```{r, error=TRUE}
grim_map(flying_pigs1, digits_x = 2)
```
...
jpap_1 |>
  grim_map(digits_x = 2) # default `items` is wrong here!
```

(Note that `5.90` and `3.50` are stored as `5.9` and `3.5`. Their trailing zeros are gone from the data, but `digits_x = 2` states that they were reported, so they are tested as two-decimal values all the same.)

Yet, all of them are consistent if the correct number of items is stated:

```{r, error=TRUE}
jpap_1 |>
  grim_map(digits_x = 2, items = 3)
```
...
Here, `digits_x` is 1 because the percentages were reported with a single decimal place. The `percent = TRUE` conversion adds the two "free" decimal places on top of it.

### Reconstructed values

Set `show_rec` to `TRUE` if you want the values that were reconstructed during GRIM-testing to be displayed in the output:

```{r}
pigs1 |>
  grim_map(digits_x = 2, show_rec = TRUE) |>
  dplyr::select(4:10) # output cut down for printing
```

The additional columns are ---

-   `rec_sum`: the unrounded product of `x` and `n` (times `items`). The sum total of integer data is a whole number, so this is generally not one.

-   `sum_lower`: the least whole-number sum total that would have been reported as `x`.

-   `sum_upper`: the greatest such sum total.

-   `rec_x_upper`: the upper reconstructed `x` value.

-   `rec_x_lower`: the lower reconstructed `x` value.

`sum_lower` and `sum_upper` are the numbers that decide the test. A value set is GRIM-consistent exactly if at least one whole number lies between them, i.e., if `sum_lower` is not greater than `sum_upper`. For an inconsistent value set the range is empty: `sum_lower` of 146 and `sum_upper` of 145 means "you would need a sum total of at least 146 but at most 145". The two are always the whole numbers on either side of `rec_sum`, so an empty range is empty by exactly one, and its width says nothing about how far off the reported numbers are. For that, compare `x` to `rec_x_lower` and `rec_x_upper`.

`rec_x_upper` and `rec_x_lower` are the two reconstructed means closest to `x`, i.e., the "granules" of the GRIM test, following the charitable and conservative protocol outlined by Brown and Heathers (2017). They are `rec_sum` rounded up and down, divided by `n` (times `items`), and they don't depend on `rounding`. The `rec_*` columns were inspired by @bauer_expression_2021's Table 1 but present values in slightly different ways.

With `percent = TRUE`, these two columns are percentages, just like `x` and just like the values returned by `grim_values()`, so that all of them can be read against each other. The three sum columns are not: a sum total of the underlying data is the same number whichever scale the mean is reported on.

Up to scrutiny 1.0.0, `show_rec` displayed the two granules re-rounded instead of the sum range, in four columns for `"up_or_down"` and the other combined rounding methods and two for the rest. This could contradict the `consistency` column it was meant to explain. All five columns above are now present for every rounding method.

### Achievable means

The sum range says how many means the reported mean could have stood for. If you want the means themselves, call `grim_values()`. It divides each whole number in that range by the sample size, so it returns the values that integer data of this size could actually have produced and that would have been reported as `x`:
...
A GRIM-inconsistent value set has none of them, and the result is empty:

```{r}
grim_values(x = 5.19, n = 28, digits_x = 2)
```

`grim_closest()` answers the complementary question: What value could have been reported instead? It returns the achievable mean nearest to `x`, whether or not `x` is consistent:

```{r}
grim_closest(x = 5.19, n = 28, digits_x = 2)
```

A sample of 28 cannot have a mean of 5.19, but it can have one of 5.178571, which would have been reported as 5.18.

Both functions are vectorized, like `grim()`. Because a reported mean can stand for any number of achievable ones, `grim_values()` returns a list, with one element per value set. That makes it easy to add as a list-column:

```{r}
pigs1 |>
  dplyr::mutate(values = grim_values(x, n, digits_x = 2))
```

Caveat: The number of achievable means grows uncontrollably once `n` (times `items`) reaches `10^digits_x`, which is exactly the point where GRIM stops being informative and the `probability` column is 0. This is also why they are not part of `grim_map()`'s output. If they were, mapping a large table in that case would produce hundreds of thousands of numbers that carry no information. `grim_closest()` is cheap there, since it never enumerates the set.
...
## Testing the values around the reported ones with `grim_map_seq()`
...
```{r}
out_seq1 <- grim_map_seq(pigs1, digits_x = 2)
out_seq1
```
...
```{r}
out_seq2 <- grim_map_seq(pigs1, digits_x = 2, dispersion = 1:10)
audit_seq(out_seq2)
```

### Visualizing the tested values
...
```{r, fig.width=6, fig.height=5.5}
out_seq1_only_x <- grim_map_seq(pigs1, digits_x = 2, var = "x")
out_seq1_only_n <- grim_map_seq(pigs1, digits_x = 2, var = "n")
...
# Detailed results:
df_tested <- grim_map_total_n(df, digits_x = c(2, 1))
df_tested
...
Note the `digits_x = c(2, 1)`: the two means were reported with different numbers of decimal places --- 4.71 with two, 5.3 with one. In a total-n mapper, a length-2 `digits_x` states the number of decimal places for `x1` and `x2` in that order. This matters here: testing 5.3 as if it were "5.30" would be a stricter test than the one Bauer and Francis conducted, and it would miss two of the three scenarios below. A single number applies to both groups.
...
`grim_probability()` takes the arguments `x`, `n`, `digits_x`, `items`, and `percent` as in `grim()` and `grim_map()`. As before, `digits_x` captures any trailing zeros:

```{r}
grim_probability(x = 1.40, n = 72, digits_x = 2)

grim_probability(x = 5.93, n = 80, digits_x = 2, items = 3)

grim_probability(x = 84.27, n = 40, digits_x = 2, percent = TRUE)
```
...
```{r}
grim_total(x = 1.40, n = 72, digits_x = 2)

grim_total(x = 5.93, n = 80, digits_x = 2, items = 3)

grim_total(x = 84.27, n = 40, digits_x = 2, percent = TRUE)
```
...
```{r}
grim_probability(x = 0.99, n = 70, digits_x = 2)
```
