...
`grim_probability()` takes the arguments `x`, `n`, `items`, and `percent` as in `grim()` and `grim_map()`:

```{r}
grim_probability(x = 1.42, n = 72)

grim_probability(x = 5.93, n = 80, items = 3)

# Enter `x` as a string to preserve trailing zeros:
grim_probability(x = "84.20", n = 40, percent = TRUE)
```
...
The formula for the probability of GRIM inconsistency is arguably implicit in Brown and Heathers' (2017) paper on GRIM. The numerator is a transformation of the formula presented on p. 364, and the authors discuss a common special case of the probability on p. 367:
