...
```{r}
debit(x = 0.35, sd = 0.18, n = 20, digits_x = 2, digits_sd = 2)
```

As in `grim()`, you need to state the number of decimal places in the mean, and here also in the SD. That is because trailing zeros can be crucial for DEBIT, but a numeric value doesn't preserve them: a mean reported as 0.30 is stored as `0.3`. Only you know how it was reported, which is why `digits_x` and `digits_sd` have no defaults.
...
flying_pigs <- tibble::tibble(
  x = c(0.4, 0.62, 0.95, 0.38, 0.96),
  sd = c(0.21, 0.13, 0.16, 0.29, 0.13),
  n = 1000,
)
...
Now, run `debit_map()` on that data frame. Both means and SDs were rounded to two decimal places, so both `digits_x` and `digits_sd` are 2:
...
## Testing the values around the reported ones with `debit_map_seq()`

DEBIT analysts might be interested in a mean or percentage value's numeric neighborhood. Suppose you found DEBIT inconsistencies, as in our example `pigs3` data. You might wonder whether they are due to small reporting or computing errors.

Use `debit_map_seq()` to use DEBIT for the values surrounding the reported means, SDs, and sample sizes:

```{r}
out_seq1 <- debit_map_seq(pigs3, digits_x = 2, digits_sd = 2)
out_seq1
```
...
out_total_n <- debit_map_total_n(out_total_n, digits_x = 2, digits_sd = 2)
out_total_n
