...
If `function_map()` is so helpful, why would you ever not use it? There are two reasons:
...
-   Any column other than `"consistency"` needs to be produced by the basic `*_scalar()` function, and to be named in the factory's `.col_names` argument. A column that is computed from the input values in some other way, such as the `probability` column in `grim_map()`'s output, has no place to come from.

`grimmer_map()` and `debit_map()` are made by `function_map()`, including their `reason` and `show_rec` columns and the `items` helper column. `grim_map()` is still "handwritten" because of that `probability` column, which does not come from `grim_scalar()`. Simply going with `function_map()` might often be the better option. If that's what you choose to do, skip right to section 4.
