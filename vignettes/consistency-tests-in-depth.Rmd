...
One caveat: `Vectorize()` copies *all* of the single-case function's formals, and it loops over all of them. If your function has an argument that only the mapper should ever set --- like `show_rec` in `grim_scalar()`, which makes it return its reconstructed values for `function_map()` to unpack into columns --- that argument ends up in the vectorized function's signature as well, where it has no business being. It is also the one argument that can make the function return something other than a single logical value, and `Vectorize()` turns that into a matrix. For the same reason, an argument like `rounding` gets looped over along with the values, so that one call can ask for two rounding methods at once.

scrutiny's own `grim()`, `grimmer()`, and `debit()` used to be made with `Vectorize()` and no longer are, for exactly these reasons. If your single-case function has such an argument, write the signature out and loop yourself.

Either way, functions created here won't be used in the remaining part of the vignette. That's because functions like `schlim()` are not great to build upon --- unlike mapper functions, which will be discussed next.
...
`function_map()` does not assume a particular number of key columns. GRIM has two, GRIMMER and DEBIT have three, and a test with five would work the same way. Normally that number is fixed when the factory runs; see *A variable number of key columns* below for the other case.
...
Some tests don't have a fixed set of key columns at all. Think of a test that checks whether the values in any number of columns add up to the value in one specific other column: how many columns go into each check is a property of the data being checked, not of the test.
...
The manufactured function then has a `parts` argument that selects columns using [tidyselect](https://tidyselect.r-lib.org/) syntax, alongside the ordinary `total` key argument:
...
-   The selected columns are returned as themselves, so the output is as rectangular as any other mapper's. Only the number of columns that go into each single test varies.

-   The variadic argument has no default and must be specified in every call. Testing all the remaining columns by default would quietly draw in any column that happens to be there for some other reason, such as an identifier.

-   The selection must not be empty, and it must not include a column that has a role in the test already --- a key column or a helper column. So `parts = everything()` is an error; write `parts = !total` instead.

-   `.reported` may be `NULL` if every key column is variadic.

-   Such a mapper is basic-tier only. `function_map_seq()` and `function_map_total_n()` derive their own arguments from `.reported`, which says nothing about the variadic columns.

### Drawbacks

If `function_map()` is so helpful, why would you ever not use it? Mainly because functions produced by `function_map()` don't have any tailor-made checks, messages, or transformations for any specific consistency test. (They do have some more general checks and error messages.)

All three of scrutiny's own basic mappers --- `grim_map()`, `grimmer_map()`, and `debit_map()` --- are made by `function_map()`, including their `reason` and `show_rec` columns, the `items` helper column, and GRIM's `probability` column, which does not come from `grim_scalar()` at all but from `.cols_derived`. Simply going with `function_map()` might often be the better option. If that's what you choose to do, skip right to section 4.
...
4.  There can be no arguments named like the key arguments unless they follow the `manage_key_colnames()` system; see there.

5.  Helper columns should be included in the output unless they transform one or more key columns. In this case, representing them in the output would be confusing because their effects have already played out via the transformed key column(s). An example is the optional `items` column in `grim_map()`'s input data frame, together with this function's `items` argument: the values are multiplied into `n`, and there is no `items` column in the output. In `function_map()`, declare helper columns with `.cols_helper`, and pair a helper column with the key column it is multiplied into using `.cols_helper_merge`. To work with helper columns in a handwritten mapper, call `manage_helper_col()`.

6.  The output data frame also includes a logical column named `"consistency"`. It contains the results of the consistency test, as determined by the respective `*_scalar()` function. In each row, `"consistency"` is `TRUE` if the values to its left are mutually consistent, and `FALSE` if they aren't. This column is placed immediately to the right of the group of key (and, potentially, helper) columns.

7.  If the underlying single-case function performs rounding or unrounding, it should internally call `reround()` and/or `unround()`, respectively. The output data frame of the mapper function will then inherit an S3 class (see section *S3 classes* below) such as `"scrutiny_rounding_up_or_down"`: It consists of `"scrutiny_rounding_"` followed by the rounding specification, e.g., `"up_or_down"`. The latter should also be the default rounding and unrounding specification. This specification can be supplied by the user via an argument called `rounding`, which is then passed down to the single-case function. If `reround()` is called within the mapper, all of its arguments need to be passed down from the mapper, which itself has all of the same arguments, with the same defaults. The same applies to `unround()`.

8.  The output data frame inherits an S3 class that starts on `"scrutiny_"`, followed by the name of the mapper function. For example, the output of `grim_map()` inherits the `"scrutiny_grim_map"` class. The `"scrutiny_"` prefix is necessary for some follow-up computations introduced below, so it should be used even within functions that are not part of scrutiny. Any other classes added to the output data frame should also start on `"scrutiny_"`. None of them should end on `"_map"`.
...
The input and output data frames are tibbles. The input data frame never contains a column named `"consistency"`, but the output data frame always does.
...
schlim_map_alt1 <- function(data, ...) {
  ...
  tibble::tibble(
    ...
    add_class("scrutiny_schlim_map") # See section "S3 classes" below
}
```
...
If your handwritten mapper creates new columns beyond `"consistency"` without deriving them from the `*_scalar()` function, you'll have to remove them from the output first.
...
If you have an eye on performance, consider building a data frame using `tibble::new_tibble()` with the `class` argument instead of `add_class()`. However, this requires you to write code that determines the number of rows in advance, so make sure you really know what you are doing.
...
If you'd like to employ such an internal helper for yourself, specify its namespace with three colons, like `scrutiny:::add_class`. However, you should only use this trick to copy and paste the helper's source code into your own source code. (That's why I left out the parentheses --- return the function itself.) Never rely on calling a function with `:::`, because these internals are not actually meant for users. They can easily shift and vanish without notice. See [this blogpost](https://tidyverse.org/blog/2022/09/playing-on-the-same-team-as-your-dependecy/) by Thomas Lin Pedersen for more information on using internal code from other packages.
...
This is an aspect of object-oriented programming (OOP), but scrutiny's use of OOP is simple even by the low standards of R. Your mapper function's output already inherits a specific class, such as `"scrutiny_grim_map"`, `"scrutiny_grimmer_map"`, or `"scrutiny_debit_map"`. In `schlim_map()`, we added the `"scrutiny_schlim_map"` class in addition to existing classes:
...
```{r}
audit_grim <- audit(grim_map(pigs1, digits_x = 2))
audit_grimmer <- audit(grimmer_map(pigs5, digits_x = 2, digits_sd = 2))
...
Predicate functions are those that return `TRUE` for the data frames returned by the factory-made functions. The more general `is_map_df()` returns `TRUE` for all those data frames.
