...
4.  Testing the values around the reported ones with `grimmer_map_seq()`.
...
```{r}
grimmer(x = 7.3, sd = 2.51, n = 12, digits_x = 1, digits_sd = 2)
```

Note `digits_x` and `digits_sd`, the numbers of decimal places in the reported mean and SD. You always need to state them, and they need to be correct: the number of decimal places is crucial for GRIMMER-testing, but a numeric value doesn't preserve it. An SD reported as 2.10 is stored as `2.1`, so scrutiny cannot tell it apart from an SD that really was reported as 2.1. Only you can, which is why these arguments have no defaults.
...
Use RStudio's multiple cursors to set commas at the end of all the values. See `vignette("wrangling")`, section *With copy and paste*, if you are not sure how to do that.

Now, run `grimmer_map()` on that data frame. The means were reported with one decimal place and the SDs with two, so `digits_x` is 1 and `digits_sd` is 2:

```{r, error=TRUE}
grimmer_map(flying_pigs1, digits_x = 1, digits_sd = 2)
```
...
flying_pigs2 |>
  grimmer_map(digits_x = 2, digits_sd = 2) # default `items` is wrong here!
```
...
```{r, error=TRUE}
flying_pigs2 |>
  grimmer_map(digits_x = 2, digits_sd = 2, items = 3)
```
...
The `items` values are multiplied by the `n` values, and the product is shown as `n` in the output. This is only for presentation; it doesn't affect the test results.
...
GRIMMER assumes that the values behind a mean and an SD are integers. Often we know more than that: responses on a five-point Likert scale are integers *between 1 and 5*. State that range with `min_val` and `max_val`, and GRIMMER will also check whether values from it could have produced the reported statistics at all.

A range caps how far the values can spread out around their mean --- and therefore how large the SD can be:
...
flying_pigs5 |>
  grimmer_map(digits_x = 2, digits_sd = 2, min_val = 1, max_val = 5)
```

All four rows are GRIMMER-consistent for unbounded integers. On a scale from 1 to 5, however:

- An SD of 2.08 is out of reach at a mean of 3. The most extreme such sample --- ten 1s and ten 5s --- has an SD of only `r round(sd(c(rep(1, 10), rep(5, 10))), 2)`.
- A mean of exactly 1 forces every single value to be 1, so the SD has to be 0.
- A mean of 7.20 is not on the scale in the first place.

Both bounds refer to a single response, so they don't depend on `items`: for a five-point scale, `min_val` is 1 and `max_val` is 5 whether the mean was composed of one item or of ten. They have to be specified together, because a single bound places no limit on the SD.

Scale bounds can only turn a `TRUE` verdict into `FALSE`, never the other way around. Like GRIMMER's other tests, the condition they add is necessary but not sufficient: a value set that passes it may still be impossible.

This is the least of what a known scale range implies, and it is in scrutiny because it costs GRIMMER one comparison per candidate sum. The [strait](https://github.com/ianhussey/strait) package is devoted to the subject and goes much further --- bounds sharpened by attained extremes, by response granularity, or by a reported Cronbach's alpha, and an exact decision procedure that is sufficient as well as necessary.
...
```{r}
flying_pigs1 |>
  grimmer_map(digits_x = 1, digits_sd = 2) |>
  audit()
```
...
5.  `fail_scale`: number of value sets that are inconsistent with the scale bounds given by `min_val` and `max_val`. Zero if these were not specified.
...
## Testing the values around the reported ones with `grimmer_map_seq()`

GRIMMER analysts might be interested in a mean or percentage value's numeric neighborhood. Suppose you found multiple GRIMMER inconsistencies as in our example `pigs5` data. You might wonder whether they are due to small reporting or computing errors.

Use `grimmer_map_seq()` to GRIMMER-test the values surrounding the reported means and sample sizes:

```{r}
out_seq1 <- grimmer_map_seq(pigs5, digits_x = 2, digits_sd = 2)
out_seq1
```
...
```{r, fig.width=6, fig.height=5.5}
out_seq1_only_x <- grimmer_map_seq(
  pigs5,
  digits_x = 2,
  digits_sd = 2,
  var = "x"
)
out_seq1_only_n <- grimmer_map_seq(
  pigs5,
  digits_x = 2,
  digits_sd = 2,
  var = "n"
...
out_total_n <- grimmer_map_total_n(flying_pigs5, digits_x = 2, digits_sd = 2)
out_total_n
