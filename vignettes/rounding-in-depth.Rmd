...
It is worth being precise about what this does and does not mean for reconstruction, because the two claims are easy to run together. `base::round()` *is* the right reconstruction of software that rounds binary doubles to even --- R itself, Python, NumPy --- and `rounding = "even"` reaches it. What is unsuitable is the reverse: predicting `round()`'s output from the decimal you see written down, since most decimals are not exactly representable and so are not really ties at all. Reconstructing the many programs that do round from the decimal 5 is the job of scrutiny's other `round_*()` functions. See `vignette("rounding-options")`, which states this once and lists which program falls on which side.
