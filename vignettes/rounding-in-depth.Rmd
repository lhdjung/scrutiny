...
The two remaining arguments are `threshold`, which concerns the obscure case of rounding from a number other than 5, and `symmetric`, which decides how ties in negative numbers are broken. `symmetric` is not obscure: with `symmetric = TRUE`, a negative number is rounded like its absolute value, which is what Excel, SPSS, SAS, and Matlab all do. If your data contain negative values and you commit to a single rounding direction, this is the setting that separates those programs from Stata and from "school math read literally on the number line". See `vignette("rounding-options")`.
...
`round_down_from()`, then, is just the reverse of `round_up_from()` --- and the reversal applies to the threshold, too. Note how the results below are the opposite way around from those above:
...
`round_up_from()` rounds up when the part cut off by rounding is at least $t$ tenths of a step. `round_down_from()` rounds *down* when that part is at most $10 - t$ tenths of a step. The two switch direction at the same point only when $t = 5$, which is the value that `round_up()` and `round_down()` use --- so the difference never shows up unless you set `threshold` yourself.
...
$$
\frac{\lceil x(10^d) - 1 + \frac{t}{10} - \epsilon \rceil}{10^d}
$$

The $\epsilon$ in both formulas is a fixed tolerance of about $1.5 \times 10^{-9}$. It is there because shifting $x$ by $d$ decimal places is not exact: $0.145 \times 100$ is stored as $14.499999999999998$, a hair below the tie it is meant to be, and rounding that shifted value directly would move $0.145$ a whole step in the wrong direction. The same tolerance is applied by every other `round_*()` function, and `unround()` reports bounds that assume it, so a number and its reconstructed range always agree. See the `Floating-point tolerance` section of `round_up()`.

### To even (base R)

Like Python's [`round()`](https://docs.python.org/3/library/functions.html#round) function, R's `base::round()` doesn't round up or down, or use any other procedure based solely on the truncated part of the number. Instead, `round()` strives to round to the next even number. This is also called "banker's rounding", and it follows a technical standard, [IEEE 754](https://en.wikipedia.org/wiki/IEEE_754#Roundings_to_nearest).
...
`base::round()` can seem like a black box, but it seems unbiased in the long run. I recommend using `round()` for original work.

For reconstruction, two claims about `round()` are easy to run together. `base::round()` is the right way to reconstruct software that rounds binary doubles to even, such as R itself, Python, and NumPy. This is what `rounding = "even"` does. What is unsuitable is the reverse: Predicting `round()`'s output from the decimal you see written down, since most decimals are not exactly representable and so are not really ties at all. Reconstructing the many programs that do round from the decimal 5 is the job of scrutiny's other `round_*()` functions. See `vignette("rounding-options")`, which lists which program falls on which side.

### IEEE 754 rounding standard

`base::round()` and some of scrutiny's rounding functions correspond to the IEEE 754 rounding standard (IEEE 2019, pp. 27f.; the [Wikipedia page](https://en.wikipedia.org/wiki/IEEE_754#Rounding_rules) is more accessible). IEEE 754 defines standards for floating-point operations, which includes rounding.
...
ggplot2::ggplot(
  df,
  ggplot2::aes(x = decimal_digits, y = bias, fill = rounding)
) +
