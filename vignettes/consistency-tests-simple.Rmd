...
This works as long as every argument of the single-case function makes sense to loop over, and the function always returns a single value. If either is not true --- as with `grim_scalar()`, whose `show_rec` argument only `grim_map()` has any use for --- write the signature out instead. See `vignette("consistency-tests-in-depth")`, section 2.
...
Use scrutiny's `audit()` generic to get summary statistics. Write a new function named `audit.scrutiny_name_map()`, where `name` is the name of your test in lower-case --- here, `schlim`.
