...
This vignette covers scrutiny's infrastructure for reconstructing rounding procedures and results from summary statistics. Doing so is essential for the package: It is a precondition for translating assumptions about the processes behind published summary data into a few simple, higher-level function calls. Some of the functions presented here might be useful beyond the package, as well.
...
First, I will present `reround()`, a general interface to reconstructing rounded numbers, before going through the individual rounding functions. In doing so, I will comment on each to some degree. That is largely to dissuade readers from using `round_up()` or `round_down()` as alternatives to `base::round()`, and to encourage them to stick with the latter in their original work.

If that sounds paradoxical, keep in mind that the goal in error detection is not to find the best approach to rounding. It is to reconstruct what other people computed with different software. This includes consistency tests, the purpose of which is to flag cases where no such reconstruction is possible.
...
Finally, I will take a closer look at bias from rounding raw numbers. That is partly because of my claims about such bias in the discussion of rounding up and to even, but also because the `rounding_bias()` function enables users to assess the extent of this problem themselves.
...
```{r}
reround(rec = c(5.812, 7.249), decimals = 2, rounding = "up")
```
...
Base R's own rounding function is an outlier in statistical computing. `base::round()` doesn't round up or down, or use any other procedure based solely on the removed part of the number. Instead, `round()` strives to round to the next even number. (This is also called "banker's rounding".)
...
A more formal take on this issue can be found below under *Rounding bias*.
...
Sometimes `round()` behaves just as it should, but at other times, results can be hard to explain. Martin Mächler, who wrote the present version of `round()`, describes the issue about as follows (<https://cran.r-project.org/web/packages/round/vignettes/Rounding.html>):
...
In effect, `base::round()` is a black box, but it is still much less biased than the alternatives, rounding up and down from 5, which come with a thick and constant inbuilt bias. `round()` can be biased in particular distributions, but it seems to be unbiased in the long run. I recommend using `round()` for original work --- even though it is quite different from other rounding procedures, and therefore unsuitable to reconstructing them. Instead, we need something like scrutiny's `round_*` functions.

Doing such reconstructive work is actually the whole purpose of functions like `round_up()` and `round_down()`. As seen above, their clarity and simplicity comes with a twist. Do not use them in place of `base::round()` unless you have to.
...
```{r}
vec4 <- rnorm(5000, 100, 15)

rounding_bias(vec4, decimals = 2)
```

What is responsible for such a difference? It seems to be (1) the sample size and (2) the number of decimal places to which the vector is rounded. Here, decimal places are rounded twice, once explicitly and once within `rounding_bias()`, to gauge that effect:
...
# Set ggplot2 theme for the remaining session:
...
ggplot2::ggplot(df, ggplot2::aes(x = dp, y = bias,
  ...
  ggplot2::labs(
    x = "Decimal places (rounding)",
    y = "Mean absolute bias"
  )

```

However, if the raw values are preliminarily rounded to 2 decimal places before rounding occurs as above, the picture is different. I will spare you the code this time:

```{r echo=FALSE}
vec5 <- round(vec4, 2)

up_1   <- rounding_bias(vec5, decimals = 1, rounding = "up")
up_2   <- rounding_bias(vec5, decimals = 2, rounding = "up")
up_3   <- rounding_bias(vec5, decimals = 3, rounding = "up")
up_4   <- rounding_bias(vec5, decimals = 4, rounding = "up")
up_5   <- rounding_bias(vec5, decimals = 5, rounding = "up")

even_1 <- rounding_bias(vec5, decimals = 1, rounding = "even")
even_2 <- rounding_bias(vec5, decimals = 2, rounding = "even")
even_3 <- rounding_bias(vec5, decimals = 3, rounding = "even")
even_4 <- rounding_bias(vec5, decimals = 4, rounding = "even")
even_5 <- rounding_bias(vec5, decimals = 5, rounding = "even")
...
df_new <- tibble::tibble(bias, dp, rounding)

df_new

# Visualize:
ggplot2::ggplot(df_new, ggplot2::aes(x = dp, y = bias,
                                     fill = rounding)) +
  ggplot2::geom_col(ggplot2::aes(fill = rounding)) +
  ggplot2::scale_y_continuous(expand = ggplot2::expansion(add = c(0, 0))) +
  ggplot2::scale_fill_discrete(name = "Rounding") +
...
In sum, the function allows users to quantify the degree to which rounding biases a distribution, so that they can assess the relative merits of different rounding procedures. This is partly to sensitize readers to the potential issues with rounding up, but also to enable them to make informed rounding decisions on their own.
