...
Functions created this way can be useful for quick testing, but unlike mapper functions, they are not great to build upon. They should never be called within other functions. Therefore, `schlim()` is not used in the remaining part of the vignette.
...
Both `grim_map()` and `debit_map()` were "handwritten" rather than produced by `function_map()`. However, none of the issues named above affect the `"consistency"` results, and simply going with `function_map()` might often be the better option. If that's what you choose to do, skip right to section 4.
...
```{r}
dummy <- tibble::tibble(x = 5)
dummy <- add_class(dummy, "dummy class")
class(dummy)
```
...
Your `audit()` method is simply a function named `audit` plus a dot and your specific class. Call `required_audit_cols()` within the method to create a tibble with the three required columns. If you don't use `required_audit_cols()`, call `check_audit_special()` in your method.
...
You can still add other summary columns to the tibble returned by `required_audit_cols()`. Use `dplyr::mutate()` or similar.
