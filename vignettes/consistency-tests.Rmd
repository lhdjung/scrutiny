...
2.  Its first argument, `data`, is a data frame that contains the key columns of the respective consistency test. Other columns are permitted but don't factor into the test unless they are helper columns. The mapper's user never needs to include helper columns and can always replace them by specifying arguments by the same names as those columns. If the user specifies such an argument but the input data frame contains a column by the same name, the function throws an error. No column of the input data frame should be named `"consistency"`.
...
4.  Helper columns should be included in the output unless they transform one or more key columns. In these cases, representing them in the output would be confusing because their effects have already played out via the transformed key column(s). An example is the optional `items` column in `grim_map()`'s input data frame. To work with helper columns, call `manage_helper_col()` within the mapper.
...
(The call to `check_mapper_input_colnames()` is not required but adds safety to your function.)
...
If your handwritten function creates new columns beyond `"consistency"`, you'll have to remove them from the output first. Don't use helper columns when testing because `function_map()` can't handle them.
...
This is an aspect of object-oriented programming (OOP), but scrutiny's use of OOP is simple even by the low standards of R. Your mapper function's output already inherits a specific class, such as `"scr_grim_map"` or `"scr_debit_map"`. In `schlim_map()`, we added the `"scr_schlim_map"` class in addition to existing classes:
