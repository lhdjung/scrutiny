...
`audit()` is an S3 generic for summarizing the results of scrutiny's test result data frames, especially those of mapper functions such as `grim_map()`. Every mapper function should have its appropriate `audit()` method.

This is an aspect of object-oriented programming (OOP), but scrutiny's use of OOP is simple even by R's standards. Your mapper function already returns a specific class, such as `"scr_grim_map"` or `"scr_debit_map"`. In `schlim_map()`, we added the `"scr_schlim_map"` class in addition to existing classes:
...
Every `audit()` method for consistency test results should be the same insofar as all consistency tests are the same. It should have a single argument named `data`. Its return value should be a tibble with a single row and at least these columns:
...
Apart from these, see for yourself which summary statistics your `audit()` method should compute.
...
```{r, eval=FALSE}
write_doc_audit(sample_output = audit(grim_map(pigs1)), name_test = "GRIM")
```

This function prepares a rogygen2 block section for you. It fills out the three standard columns for you, and it leaves space to describe the other columns. Also, the internal checks of `write_doc_audit()` make sure that you wrote a correct `audit()` method, as represented by the value of the `sample_output` argument.
...
It is hard to give general advice on how to create visualization functions for the results of consistency tests. As with the `_scalar` function, the implementation of a visualization function greatly depends on the idiosyncratic nature of the consistency test itself. When comparing the looks of `grim_plot()` and `debit_plot()`, it is clear that two very different things are going on.

## Requirements

Nevertheless, some requirements do apply to scrutiny-style visualization functions. They are much more like arbitrary conventions than the requirements for mapper functions, which often meet very particular technical needs. Visualization functions are also dead ends for programming: No other computations build up on them, at least not within scrutiny.

As a result, the points below are admittedly somewhat less important. If you violate them, nobody but me will be sad about it.
...
5.  The most important aesthetics for the geom layers are passed down to the latter via arguments. This namely concerns `alpha` (i.e. opacity), `color`, and `size`, in this order. If the respective colors are covered by `color_cons` and `color_incons`, not further `color` argument is needed for them. If multiple layers need `alpha`, `color`, and `size` arguments, the latter should start with an expression that stands for the layer, not with `alpha`, `color`, or `size`, respectively. Examples are `line_alpha`, `line_color`, and `line_size` in `debit_plot()`.
6.  Arguments passed on to aesthetics should have the same defaults as those aesthetics themselves have in ggplot2 unless there is a compelling reason for different defaults. Again, `line_alpha`, `line_color`, and `line_size` are examples for this.
...
### Producing functions with `function_map_total_n()`

`function_map_total_n()` creates new functions which follow this very scheme by applying a given consistency test to multiple combinations of reported and hypothetical summary statistics. It is the powerhouse behind `grim_map_total_n()` and `debit_map_total_n()`. It's not a helper used inside other functions when creating them with `function()` --- instead, it takes the place of `function()` itself.

See the [case study in the GRIM vignette](https://lhdjung.github.io/scrutiny/articles/grim.html#handling-unknown-group-sizes-with-grim_map_total_n) for an example of how `grim_map_total_n()` works out in practice. Although the underlying code is fairly complex, the function itself was written in a very simple way:
...
The same pattern can be applied to any function like `grim_map()`, i.e., a function that tests the values contained in multiple columns of a data frame for consistency. One of the columns, `n`, will have its values dispersed from half using `disperse_total()`, explained below.

To export such a function from your own package, make sure to follow [this purrr FAQ](https://purrr.tidyverse.org/reference/faq-adverbs-export.html). (Incredible as it sounds, scrutiny will then take on the role of purrr.) Also, see this [worked example](https://lhdjung.github.io/scrutiny/reference/function_map_total_n.html#ref-examples) for the `.onLoad` pattern.
