...
This function prepares a rogygen2 block section for you. It fills out the obligatory columns for you, and it leaves space to describe the other columns. Also, the internal checks of `write_doc_audit()` make sure that you wrote a correct `audit()` method, as represented by the value of the `sample_output` argument.
...
2.  The first argument, `data`, is a data frame that is the results of a call to the respective mapper function, such as `grim_map()` or `debit_map()`. The visualization function makes sure this is true by checking that `data` inherits the special class added within the mapper, such as `scr_grim_map` or `scr_debit_map`. If `data` fails this check, the function throws an error.
...
5.  The most important aesthetics for the geom layers are passed down to the latter via arguments. This namely concerns `alpha` (i.e. opacity), `color`, and `size`, in this order. If the respective colors are covered by `color_cons` and `color_incons`, not further `color` argument is needed. If multiple layers need `alpha`, `color`, and `size` arguments, the latter should start with an expression that stands for the layer, not with `alpha`, `color`, or `size`, respectively. Examples are `line_alpha`, `line_color`, and `line_size` in `debit_plot()`.
6.  Arguments passed on to aesthetics should have the same defaults as those aesthetics themselves have in ggplot2 unless therre is a compelling reason for different defaults. Again, `line_alpha`, `line_color`, and `line_size` are examples for this.
