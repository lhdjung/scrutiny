...
This vignette provides an overview of scrutiny's miscellaneous infrastructure for implementing error detection techniques. For more specific article, see `vignette("rounding")` or `vignette("consistency-tests")`.
...
`base::seq()` is an elegant way to generate regular sequences, but it is almost too elegant: The way to determine the step size entirely depends on which arguments are added with non-standard evaluation (i.e., via the dots, `…`).
...
There is one last pair of functions to create decimal-level sequences: `seq_disperse()` and `seq_disperse_df()`. They also start at `from` but don't simply proceed forward. Instead, they construct a sequence centered around `from`:

```{r}
seq_disperse(from = 0.25)
```

A key argument here is `dispersion`, which controls the steps up and down. The default is `1:5` for five steps in each direction, but it's very flexible:

```{r}
seq_disperse(from = 0.25, dispersion = 1:2)

seq_disperse(from = 0.25, dispersion = c(1, 3, 5))

seq_disperse(from = 0.25, dispersion = 7:9)
```

The data frame variant is just what you would expect, except it can track the changes to `.from`:

```{r}
seq_disperse_df(.from = 0.25, .dispersion = 1:2, .track_var_change = TRUE)
```

These functions are hybrids between the `seq()` wrappers explained above and the `disperse*()` functions introduced next.

## Vary hypothetical group sizes

TO DO: Port this entire section to the *Implementing consistency tests* vignette, except for the part on `disperse_total()`, because only this latter part really is general infrastructure!
...
You might want to write an `audit()` method for the specific class inherited by the output of your manufactured function, or a method for a different generic. If so, `summarize_map_total_n()` will be of interest to you: This one really is a helper.
