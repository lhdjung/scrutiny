...
Four predicate functions around `is_seq_linear()` test whether a vector `x` represents particular kinds of sequences. They are listed as infrastructure because the sequence generators above are, but these testing functions can serve as analytic instruments in their own right.
...
is_seq_descending(x = 16:9)

# Default also tests for linearity:
is_seq_descending(x = c(16:9, 2))
is_seq_descending(x = c(16:9, 2), test_linear = FALSE)
```
