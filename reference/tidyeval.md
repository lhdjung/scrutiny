...
This page lists the tidy eval tools reexported in scrutiny from rlang.
To learn about using tidy eval in scripts and packages at a high level,
see the [dplyr programming
...
- `.data` is a pronoun that should only be used within tidy eval
  functions.

- `:=` has no return value. It will throw an error if called outside of
  the dynamic dots of a tidy eval function.
